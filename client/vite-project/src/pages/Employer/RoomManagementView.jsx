  ...
  const fetchJob = useCallback(async () => {
    try {
      ...
      if (!response.data.success || !response.data.job) {
        setError(response.data.message || "Failed to load room");
        return;
      }
        ...
        <div className="flex items-center justify-between gap-3">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-gray-500">View and manage your listed room</p>
            </div>
          </div>
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
              <div className="rounded-xl border border-gray-100 p-4">
                <p className="text-xs text-gray-500 flex items-center gap-1.5">
                  <p className="h-3.5 w-3.5" /> Price Range
                </p>
                <p className="text-sm font-medium text-gray-900 mt-1">
                  NPR {job.salaryMin?.toLocaleString()} - NPR {job.salaryMax?.toLocaleString()} / night
                </p>
              </div>
              <div className="rounded-xl border border-gray-100 p-4">
                <p className="text-xs text-gray-500 flex items-center gap-1.5">
              ...
              <div className="rounded-xl border border-gray-100 p-4">
                <p className="text-xs text-gray-500 flex items-center gap-1.5">
                  <Clock className="h-3.5 w-3.5" /> Listed On
                </p>
            ...
            <div>
              <h3 className="text-sm font-semibold text-gray-900 flex items-center gap-1.5">
                <Briefcase className="h-4 w-4" /> Room Description
              </h3>
