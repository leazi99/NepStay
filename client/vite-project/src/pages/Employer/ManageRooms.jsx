...
const ManageJobs = () => {
  ...
  const fetchJobs = async () => {
    try {
      setIsLoading(true);
      setError("");
      const res = await axiosInstance.get(API_PATHS.ROOMS.GET_BY_HOTEL("me"));
      if (res.data.success) {
        setJobs(res.data.rooms || []);
      } else {
  ...
  const handleToggleClose = async (jobId) => {
    setToggling(jobId);
    try {
      const res = await axiosInstance.put(API_PATHS.ROOMS.UPDATE(jobId), { isClosed: true });
      if (res.data.success) {
        setJobs((prev) =>
        ...
        toast.error(res.data.message || "Failed to toggle room status");
      }
    } catch {
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      const res = await axiosInstance.delete(API_PATHS.ROOMS.DELETE(confirmDelete.jobId));
      if (res.data.success) {
    ...
    <DashboardLayout activeMenu="manage-rooms">
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            <h1 className={`text-2xl font-bold ${isDark ? "text-slate-100" : "text-gray-900"}`}>Manage Rooms</h1>
            <p className={`text-sm mt-1 ${isDark ? "text-slate-400" : "text-gray-500"}`}>{jobs.length} room{jobs.length !== 1 ? "s" : ""} listed</p>
          </div>
          <button
        ...
        {isLoading ? (
          ...
          <div className={`flex flex-col items-center justify-center rounded-2xl border py-20 gap-4 ${isDark ? "bg-slate-900 border-slate-700 text-slate-400" : "bg-white border-gray-100 text-gray-400"}`}>
            ...
            <p className="text-sm">{search ? "No rooms match your search" : "No rooms listed yet"}</p>
            {!search && (
              <button
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className={`${isDark ? "bg-slate-800 border-slate-700" : "bg-gray-50 border-gray-100"} border-b`}>
                  <th className={`text-left px-6 py-3.5 text-xs font-semibold uppercase tracking-wide ${isDark ? "text-slate-400" : "text-gray-500"}`}>Room</th>
                  ...
                  <th className={`text-center px-6 py-3.5 text-xs font-semibold uppercase tracking-wide hidden md:table-cell ${isDark ? "text-slate-400" : "text-gray-500"}`}>Reservations</th>
                  <th className={`text-left px-6 py-3.5 text-xs font-semibold uppercase tracking-wide hidden lg:table-cell ${isDark ? "text-slate-400" : "text-gray-500"}`}>Added</th>
                ...
                {filtered.map((job) => (
                  <tr key={job._id} className={`transition-colors ${isDark ? "hover:bg-slate-800/60" : "hover:bg-gray-50"}`}>
                    <td className="px-6 py-4">
                      ...
                      <p className={`text-xs mt-0.5 ${isDark ? "text-slate-400" : "text-gray-400"}`}>NPR {job.pricePerNight?.toLocaleString()}/night</p>
                      <p className={`text-xs mt-0.5 ${isDark ? "text-slate-400" : "text-gray-400"}`}>Location: {job.city || "Not specified"}</p>
                    </td>
                    ...
                    <td className="px-6 py-4 text-center hidden md:table-cell">
                      <button
                        onClick={() => navigate(`/manager/reservations?room=${job._id}`)}
                      ...
                      >
                        ...
                        <span className="font-medium">{job.bookingCount || 0}</span>
                      </button>
                    </td>
                    ...
                    <td className="px-6 py-4">
                      <div className="flex items-center justify-end gap-1">
                        <ActionIconButton
                          label="View Room"
                          onClick={() => navigate(`/room/${job._id}/manage`)}
                        ...
                        <ActionIconButton
                          label="Edit Room"
                          onClick={() => navigate(`/manage-rooms/${job._id}/edit`)}
                        ...
                        <ActionIconButton
                          label={job.isClosed ? "Reopen Room" : "Close Room"}
      ...
      <ConfirmDialog
        ...
        message="This will permanently remove the room listing. This action cannot be undone."
