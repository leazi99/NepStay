...
const EmployerDashboard = () => {
  ...
  const onboardingItems = [
      {
        ...
        action: () => navigate("/manage-rooms/new"),
      },
    {
      key: "firstApplicant",
    ...
    {
      key: "firstHire",
      label: "Confirm first reservation",
      done: Number(counts?.totalHired || 0) > 0,
      actionLabel: "View Reservations",
        action: () => navigate("/manager/reservations"),
    },
    {
      key: "companyProfile",
      label: "Complete hotel profile",
      done: Boolean(
        recentJobs.length > 0 ||
          recentApplications.length > 0 ||
          analytics?.companyProfileCompleted,
      ),
      actionLabel: "Hotel Profile",
  ...
  const getFallbackAnalytics = useCallback(async () => {
    ...
    if (!jobsResponse?.data?.success) {
      throw new Error(jobsResponse?.data?.message || "Failed to load rooms");
    }
    ...
    <div className="max-w-7xl mx-auto space-y-8">
      ...
      <div className="flex items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">Track your booking activity and performance</p>
        </div>
      ...
      <div className="bg-white rounded-2xl border border-gray-100 p-5 sm:p-6 shadow-sm">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-4">
          <div>
            ...
            <p className="text-xs sm:text-sm text-gray-500 mt-1">Complete these steps to get your hotel listing ready.</p>
          </div>
        ...
        <div className="h-2 rounded-full bg-gray-100 overflow-hidden mb-5">
          <div className="h-full bg-blue-600 transition-all duration-500" style={{ width: `${onboardingProgress}%` }} />
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          {onboardingItems.map((item) => (
            <div
              key={item.key}
              className={`rounded-xl border px-4 py-3 flex items-center justify-between gap-3 ${item.done ? "border-emerald-200 bg-emerald-50" : "border-gray-200 bg-white"}`}
            >
              ...
              {!item.done && (
                <button onClick={item.action} className="text-xs font-medium text-blue-600 hover:text-blue-700 whitespace-nowrap">
                  {item.actionLabel}
                </button>
              )}
            </div>
          ...
          <div className="divide-y divide-gray-50">
            {recentJobs.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-12 text-gray-400 gap-3">
                ...
                <button onClick={() => navigate("/manage-rooms/new")} className="text-xs text-blue-600 hover:underline">
                  Add your first room →
                </button>
              </div>
            ) : (
              recentJobs.map((job) => (
                <div key={job._id} className="flex items-center justify-between px-6 py-4 hover:bg-gray-50 transition-colors">
          ...
          <div className="divide-y divide-gray-50">
            {recentApplications.length === 0 ? (
              ...
              recentApplications.map((app) => (
                <div key={app._id} className="flex items-center gap-4 px-6 py-4 hover:bg-gray-50 transition-colors">
                  <div className="h-9 w-9 rounded-full bg-gradient-to-br from-blue-100 to-blue-200 flex items-center justify-center flex-shrink-0">
                    {app.applicant?.avatar ? (
                      <img src={app.applicant.avatar} alt={app.applicant.name} className="h-9 w-9 rounded-full object-cover" />
                    ) : (
                  ...
                  <div className="min-w-0 flex-1">
                    <p className="text-sm font-medium text-gray-900 truncate">{app.applicant?.name || "Unknown"}</p>
                    <p className="text-xs text-gray-400 truncate">
                      Reserved <span className="text-gray-600 font-medium">{app.job?.title}</span>
                    </p>
                  </div>
