...
const UserProfile = () => {
  ...
  const profileStatus = useMemo(() => {
    ...
    const items = [
      ...
      { key: "preferences", label: "Stay Preferences", completed: hasInterests && hasEducation && hasSpecialization },
      { key: "resume", label: "Identity Document", completed: hasResume },
      { key: "student_id", label: "Verification Document", completed: hasStudentId },
    ...
    <div className={`min-h-screen ${isDark ? "bg-slate-900" : "bg-gray-50"}`}>
      ...
      <div className={`py-12 px-4 ${isDark ? "bg-slate-950" : "bg-gradient-to-r from-blue-700 via-blue-600 to-indigo-600"}`}>
        <div className="max-w-4xl mx-auto">
          <div>
            <h1 className="text-3xl font-bold text-white">Guest Profile</h1>
            <p className="text-sm mt-2 text-blue-100">Update your details, documents, bio, preferences and password</p>
          </div>
        </div>
      </div>
        ...
        <form onSubmit={handleSave} className="space-y-6">
          <div className={`rounded-2xl border shadow-sm p-6 ${cardClass}`}>
            ...
            <div className="flex items-center gap-6">
              ...
              <div>
                ...
                <p className={`text-sm capitalize ${textSecondary}`}>Guest</p>
          ...
          <div className={`rounded-2xl border shadow-sm p-6 ${cardClass}`}>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div className="sm:col-span-2">
                <InputField
                  label="Stay Preferences (comma separated)"
                  name="interests"
                  placeholder="Balcony, Breakfast, Ocean View"
          ...
          <div className={`rounded-2xl border shadow-sm p-6 space-y-4 ${cardClass}`}>
            ...
            {form.resume ? (
              <div className={`flex items-center gap-3 p-4 border rounded-xl ${isDark ? "bg-green-900/20 border-green-700" : "bg-green-50 border-green-200"}`}>
                ...
                <div className="flex-1 min-w-0">
                  <p className={`text-sm font-medium ${isDark ? "text-green-300" : "text-green-800"}`}>Document uploaded</p>
                  <a
                    href={form.resume}
                    target="_blank"
                    rel="noreferrer"
              ...
              <div className={`border-2 border-dashed rounded-xl p-6 text-center transition-colors ${isDark ? "border-slate-600" : "border-gray-200"}`}>
                ...
                <p className={`text-sm mb-3 ${isDark ? "text-slate-300" : "text-gray-500"}`}>Upload your document (PDF, DOC)</p>
                <label className="cursor-pointer">
                  <span className="inline-flex items-center gap-2 px-4 py-2 bg-blue-600 text-white text-sm font-medium rounded-xl hover:bg-blue-700 transition-colors">
                    ...
                    {resumeUploading ? "Uploading..." : "Upload Document"}
                  </span>
            ...
            <div className="flex flex-col gap-1.5">
              <label className={`text-sm font-medium ${isDark ? "text-slate-200" : "text-gray-700"}`}>Or paste document link</label>
              <input
                type="url"
                name="resume"
          ...
          <div className={`rounded-2xl border shadow-sm p-6 space-y-4 ${cardClass}`}>
            ...
            {form.studentIdCard ? (
              ...
              <div className={`border-2 border-dashed rounded-xl p-6 text-center transition-colors ${isDark ? "border-slate-600" : "border-gray-200"}`}>
                ...
                <p className={`text-sm mb-3 ${isDark ? "text-slate-300" : "text-gray-500"}`}>Upload your verification document (PDF, JPG, PNG)</p>
                <label className="cursor-pointer">
                  <span className="inline-flex items-center gap-2 px-4 py-2 bg-blue-600 text-white text-sm font-medium rounded-xl hover:bg-blue-700 transition-colors">
                    {studentIdUploading ? (
                      <Loader2 className="h-4 w-4 animate-spin" />
                    ...
                    {studentIdUploading ? "Uploading..." : "Upload Document"}
                  </span>
            ...
            <div className="flex flex-col gap-1.5">
              <label htmlFor="student-id-link" className={`text-sm font-medium ${isDark ? "text-slate-200" : "text-gray-700"}`}>Or paste document link</label>
              <input
                id="student-id-link"
                type="url"
                name="studentIdCard"
          ...
          <>
            <div className={`rounded-2xl border shadow-sm p-6 ${cardClass}`}>
              <h2 className={`font-semibold text-base mb-3 ${textPrimary}`}>
                Rate Hotels (Completed Stays)
              </h2>
              {eligibleReviews.length === 0 ? (
                <p className={`text-sm ${textSecondary}`}>No pending hotel reviews.</p>
              ) : (
                    ...
                    return (
                      ...
                      >
                        <p className={`text-sm font-medium ${textPrimary}`}>
                          {item.reviewee?.name || "Hotel"}
                        </p>
                        <p className={`text-xs mb-2 ${textSecondary}`}>{item.job?.title || "Stay"}</p>
            ...
            <div className={`rounded-2xl border shadow-sm p-6 ${cardClass}`}>
              <h2 className={`font-semibold text-base mb-1 ${textPrimary}`}>Your Guest Reviews</h2>
                ...
                <div className="space-y-3">
                  {receivedReviews.slice(0, 5).map((review) => (
                    ...
                    >
                      ...
                      <p className={`text-xs ${textSecondary}`}>{review.job?.title || "Stay"}</p>
                      {review.comment ? (
