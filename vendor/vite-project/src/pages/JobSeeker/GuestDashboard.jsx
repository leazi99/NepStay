...
import { CATEGORIES, roomTypes } from "../../utils/data";
import FreelancerNavbar from "../../components/layout/FreelancerNavbar";

const EXPERIENCE_LEVELS = ["Entry Level", "Intermediate", "Expert"];
const BUDGET_RANGES = [
  { label: "Less than NPR 2,500", min: 0, max: 2500 },
  { label: "NPR 2,500 to NPR 7,500", min: 2500, max: 7500 },
  { label: "NPR 7,500 - NPR 15,000", min: 7500, max: 15000 },
  { label: "More than NPR 15,000", min: 15000, max: Infinity },
];
...
const JobListItem = ({ job, onSave, onUnsave, onApply, onDislike, nowTs, isDark }) => {
  ...
  const bookingText = typeof job.applicationCount === "number" ? `Active Bookings: ${job.applicationCount}` : "";
  ...
  const postedText = () => {
    ...
    if (minutes < 60) return `Listed ${Math.max(minutes, 1)} minute${minutes === 1 ? "" : "s"} ago`;
  ...
  return (
    ...
    >
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-sm font-semibold text-blue-600 dark:text-blue-400">
            {job.location} Room &bull; NPR {estimateBudget.toLocaleString()}/night
          </p>
        </div>
      ...
      <div className={`mt-4 pt-4 border-t text-xs flex flex-wrap items-center gap-x-3 gap-y-2 ${isDark ? "border-slate-700 text-slate-400" : "border-gray-200 text-gray-500"}`}>
        ...
        <span className="inline-flex items-center gap-1 text-blue-400"><BadgeCheck className="h-4 w-4" /> Booking verified</span>
        ...
        {companySpent ? <span>{companySpent} spent</span> : null}
      ...
      {bookingText ? (
        <p className={`mt-3 text-xs ${isDark ? "text-slate-400" : "text-gray-500"}`}>{bookingText}</p>
      ) : null}
      ...
      <div className="mt-4" onClick={(event) => event.stopPropagation()}>
        {normalizedStatus ? (
          ...
          >
            {normalizedStatus === "Accepted" ? "Confirmed" : normalizedStatus === "Rejected" ? "Cancelled" : "Pending"}
          </span>
        ) : (
          <button
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[280px_1fr] gap-6 items-start">
          <aside className={`lg:sticky lg:top-24 rounded-2xl border p-5 ${isDark ? "bg-slate-900 border-slate-700" : "bg-white border-gray-200"}`}>
            <div className="space-y-5">
              <div>
                <p className={`text-xs uppercase tracking-wide mb-2 ${isDark ? "text-slate-400" : "text-gray-500"}`}>Room Category</p>
                <select
              ...
              <div>
                <p className={`text-xs uppercase tracking-wide mb-2 ${isDark ? "text-slate-400" : "text-gray-500"}`}>Hotel Location</p>
                <input
                  type="text"
              ...
              <div>
                <p className={`text-xs uppercase tracking-wide mb-2 ${isDark ? "text-slate-400" : "text-gray-500"}`}>Room Type</p>
              ...
              <div>
                <p className={`text-xs uppercase tracking-wide mb-2 ${isDark ? "text-slate-400" : "text-gray-500"}`}>Budget (per night)</p>
                <div className="space-y-2">
              ...
              <div>
                <p className={`text-xs uppercase tracking-wide mb-2 ${isDark ? "text-slate-400" : "text-gray-500"}`}>Custom Price Range</p>
              ...
              <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
                <div className="flex-1 max-w-2xl">
                  <div className="relative">
                    ...
                    <input
                      ...
                      placeholder="Search rooms by name, category, or location..."
                ...
                <div className="flex items-center gap-2 self-end md:self-auto">
                  ...
                  {dislikedJobIds.length > 0 && (
                    ...
                    >
                      Reset hidden rooms ({dislikedJobIds.length})
                    </button>
                  )}
                  <div className="relative">
                    <select
                    ...
                    >
                      ...
                      <option value="salary_high">Sort by: Price High</option>
                      <option value="salary_low">Sort by: Price Low</option>
                    </select>
              ...
              {!isAuthenticated ? (
                <div className={`mt-3 rounded-xl border px-3 py-3 flex items-center justify-between ${isDark ? "border-slate-700 bg-slate-800" : "border-gray-200 bg-gray-50"}`}>
                  <p className={`text-sm ${isDark ? "text-slate-300" : "text-gray-600"}`}>
                    Login to see rooms you have booked.
                  </p>
                  <button
                      ...
                      >
                        <div>
                          <p className={`text-sm font-semibold ${isDark ? "text-slate-100" : "text-gray-900"}`}>
                            {application.job?.title || "Room not available"}
                          </p>
                          <p className={`text-xs mt-1 ${isDark ? "text-slate-400" : "text-gray-500"}`}>
                            {companyName} &bull; Booked on {formatAppliedDate(application.createdAt)}
                          </p>
                        </div>

                        <div className="flex items-center gap-2">
                          <span className={`px-2.5 py-1 rounded-full text-xs font-semibold ${getApplicationBadgeClasses(status)}`}>
                            {status === "Accepted" ? "Confirmed" : status === "Rejected" ? "Cancelled" : "Pending"}
                          </span>
                          {application.job?._id ? (
                            <button
                              onClick={() => navigate(`/room/${application.job._id}`)}
                              className="px-3 py-1.5 rounded-lg text-xs font-medium bg-blue-600 text-white hover:bg-blue-700 transition-colors"
                            >
                              View Room
                            </button>
