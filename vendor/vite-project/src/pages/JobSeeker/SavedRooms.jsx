  ...
  const fetchSaved = async () => {
    try {
      ...
      if (res.data.success) {
        ...
        setError(res.data.message || "Failed to load saved rooms");
      }
    } catch {
  ...
  return (
    <div className={`min-h-screen ${isDark ? "bg-slate-900" : "bg-gray-50"}`}>
      ...
      <div className="bg-gradient-to-r from-blue-700 via-blue-600 to-indigo-600 py-12 px-4 text-center">
        <h1 className="text-3xl font-bold text-white">Saved Rooms</h1>
        <p className="text-blue-100 text-sm mt-2">Rooms you've bookmarked for later</p>
      </div>
      ...
      <div className="max-w-4xl mx-auto px-4 py-10 space-y-5">
        {isLoading ? (
          ...
          <>
            <p className={`text-sm ${isDark ? "text-slate-300" : "text-gray-500"}`}>{savedJobs.length} saved room{savedJobs.length !== 1 ? "s" : ""}</p>
            {savedJobs.map((saved) => {
              const job = saved.job;
              if (!job) return null;
