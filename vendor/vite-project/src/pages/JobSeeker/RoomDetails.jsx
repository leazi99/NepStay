...
const JobDetails = () => {
  ...
  }, [resolveThemePreference, user?.themePreference]);
  ...
  if (error || !job) {
    return (
      <div className={`min-h-screen flex flex-col items-center justify-center gap-4 ${isDark ? "bg-slate-950" : "bg-gray-50"}`}>
        ...
        <p className="text-red-500 text-base">{error || "Room not found"}</p>
        <button onClick={() => navigate("/guest-dashboard")} className="px-4 py-2 rounded-lg bg-red-600 text-white hover:bg-red-700 transition-colors">
          Back to Rooms
        </button>
      </div>
    );
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                ...
                <p className={`mt-1 text-xs inline-flex items-center gap-1.5 ${companyRatingCount > 0 ? "text-amber-500" : isDark ? "text-slate-400" : "text-gray-500"}`}>
                  ...
                  {companyRatingCount > 0
                    ? `${companyRatingAvg.toFixed(1)} guest rating (${companyRatingCount} reviews)`
                    : "New hotel (no public reviews yet)"}
                </p>
